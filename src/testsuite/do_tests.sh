...
TEST_BASEDIR=${NEST_DOC_DIR:-"/tmp/tmp1/share/doc/nest"}
...
PYTHON_HARNESS="${NEST_DATA_DIR:-/tmp/tmp1/share/nest}/extras/do_tests.py"
...
echo >  "${TEST_LOGFILE}" "NEST v. nest-3@7c9ab01 testsuite log"
echo >> "${TEST_LOGFILE}" "======================"
...
if test "x$(sli -c 'statusdict/have_music :: =')" = xtrue ; then
    ...
    for test_name in $(ls ${TESTDIR} | grep '.*\.music$') ; do
        ...
        command=$(sli -c "${np} () (${test_name}) mpirun =")
...
if test "x${TEST_PYNEST}" = xtrue ; then
    ...
    if command -v  >/dev/null 2>&1 &&   --with-xunit --xunit-file=/dev/null --where=${TEST_OUTDIR} >/dev/null 2>&1; then

        echo
        echo "  Using nosetests."
        echo

          -v --with-xunit --xunit-file=${TEST_OUTDIR}/pynest_tests.xml ${NEST_PYTHON_PREFIX:-/tmp/tmp1/}/nest/tests 2>&1 \
